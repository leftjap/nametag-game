...
### Step 1
- 파일: [절대 경로] (예: C:\dev\apps\keep\js\ui.js)
...
- 모든 Step의 `파일:` 항목에 절대 경로를 사용한다. 상대 경로(`js/ui.js`)가 아닌 전체 경로(`C:\dev\apps\keep\js\ui.js`)를 명시한다. Haiku가 프로젝트 위치를 찾지 못하는 것을 방지한다.
...
- 갱신 후 별도 커밋:
  ...
  git push origin main
  ```
- playbook.md 크롤링 경로: `https://raw.githubusercontent.com/leftjap/playbook/main/playbook.md`
...
```
### Step N: GAS 배포
- 파일: C:\dev\apps\keep\gas\Code.js
- 작업:
  ```bash
  cd "C:\dev\apps\keep\gas"
  clasp push
  ```
...
**경로:** 메인 레포(`nametag`) 내 하위 폴더. `C:\dev\apps\keep\gas\`
...
**PowerShell 치환 예시:**
```powershell
$file = 'C:\dev\apps\keep\AGENTS.md'
...
e. 배포 URL(https://leftjap.github.io/keep/)로 접속 상태를 확인한다 (200 vs 404).
...
사용할 것:
  `raw.githubusercontent.com/leftjap/keep/main/{파일경로}`
