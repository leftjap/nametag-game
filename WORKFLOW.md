...
| GAS 서버 수정 | nametag(gas)/Code.gs | sync.js |
...
**Code.gs만 수정하고 메인 레포 파일은 변경하지 않은 경우**, Git 커밋 & 푸시 Step은 생략한다. nametag(gas) 폴더는 별도 프로젝트이고 메인 레포의 Git 관리 대상이 아니다.
...
nametag(gas)/          — Google Apps Script (메인 레포와 별도 폴더)
