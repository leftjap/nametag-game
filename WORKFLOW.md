...
**화살표 예외:** 글쓰기 메뉴(`.side-menu`)는 화살표 대신 글 개수(`.badge-pill`)를 표시한다. 가계부(`.expense-compact`)는 화살표 대신 금액(`.expense-compact-amount`)만 표시한다. 이 두 항목에 `.side-arrow`를 추가하지 않는다. CSS에서 `.side-menu .side-arrow`와 `.expense-compact .side-arrow`에 `display: none !important`가 적용되어 있다.
